...
export function IdentityAccess() {
  ...
  const [form, setForm] = useState({ email: '', role: 'org:inspector', note: '' });
  ...
  return <div className="admin-page identity-page">
    ...
    <section className="identity-layout">
      ...
      <div className="identity-content">
        <article className="identity-card"><header><Users size={18} /><div><h2>Approval queue</h2><p>No invitation is sent until you approve it.</p></div></header>{requests.length ? requests.map((item) => <div className="identity-row" key={item.request_id}><div><strong>{item.email}</strong><small>{roleLabel(item.role)} · {clients.find((client) => client.org_id === item.org_id)?.name || item.org_id}</small></div><div><button disabled={busy} onClick={() => void act(() => request(`/v1/admin/access-requests/${item.request_id}/approve`, { method: 'POST' }))}><Check size={14} /> Approve</button><button className="reject" disabled={busy} onClick={() => void act(() => request(`/v1/admin/access-requests/${item.request_id}/reject`, { method: 'POST' }))}><X size={14} /> Reject</button></div></div>) : <div className="identity-empty"><ShieldCheck size={26} /><span>No people are waiting for approval.</span></div>}</article>
        {orgId && <><article className="identity-card"><header><ShieldCheck size={18} /><div><h2>Request access</h2><p>Add the person to your private approval queue.</p></div></header><div className="identity-form"><label><span>Email address</span><input type="email" value={form.email} placeholder="person@company.com" onChange={(event) => setForm({ ...form, email: event.target.value })} /></label><label><span>Role</span><select value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value })}>{roles.map((role) => <option key={role.key} value={role.key}>{role.label}</option>)}</select></label><label className="wide"><span>Note (optional)</span><input value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} /></label><button className="primary-button" disabled={busy || !form.email} onClick={() => void submit()}>{busy ? <LoaderCircle className="spinner" size={15} /> : <Users size={15} />} Send for approval</button></div></article><article className="identity-card"><header><Users size={18} /><div><h2>Active members</h2><p>Role changes are applied directly to Clerk.</p></div></header>{members.map((member) => { const user = member.public_user_data || {}; return <div className="identity-row" key={member.id}><div><strong>{user.identifier || user.user_id}</strong><small>{user.user_id}</small></div><div><select disabled={busy || member.role === 'org:admin'} value={member.role} onChange={(event) => void act(() => request(`/v1/admin/clients/${encodeURIComponent(orgId)}/members/${user.user_id}`, { method: 'PATCH', body: JSON.stringify({ role: event.target.value }) }))}>{!roles.some((role) => role.key === member.role) && <option value={member.role}>{roleLabel(member.role)}</option>}{roles.map((role) => <option key={role.key} value={role.key}>{role.label}</option>)}</select><button className="reject" disabled={busy} onClick={() => window.confirm('Remove this person from the client organization?') && void act(() => request(`/v1/admin/clients/${encodeURIComponent(orgId)}/members/${user.user_id}`, { method: 'DELETE' }))}><UserMinus size={14} /> Remove</button></div></div>; })}{!members.length && <div className="identity-empty"><Users size={26} /><span>No members returned by Clerk.</span></div>}</article></>}
      </div>
    </section>
